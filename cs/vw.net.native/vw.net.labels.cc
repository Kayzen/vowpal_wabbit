...
API float SimpleLabelReadFromExample(VW980::example* ex, float& weight, float& initial)
{
  auto* ld = &ex->l.simple;
...
API void SimpleLabelUpdateExample(vw_net_native::workspace_context* workspace, VW980::example* ex, float label,
    float* maybe_weight, float* maybe_initial)
{
  ...
  VW980::count_label(*workspace->vw->sd, ld->label);
}

API VW980::cb_class* CbLabelReadFromExampleDangerous(VW980::example* ex)
{
...
API void CbLabelUpdateExample(VW980::example* ex, const VW980::cb_class* f)
{
  VW980::cb_label* ld = &ex->l.cb;
...
API vw_net_native::ERROR_CODE StringLabelParseAndUpdateExample(vw_net_native::workspace_context* workspace, example* ex,
    const char* label, size_t label_len, VW980::experimental::api_status* status)
{
  std::string label_str(label, label_len);

  try
  {
    VW980::parse_example_label(*workspace->vw, *ex, label_str);
  }
  CATCH_RETURN_STATUS
...
API uint32_t SharedLabelGetActionId() { return (uint32_t)VW980::uniform_hash("shared", 6, 0); }

API char* ComputeDiffDescriptionSimpleLabels(VW980::example* ex1, VW980::example* ex2)
{
  auto* ld1 = &ex1->l.simple;
  auto* ld2 = &ex2->l.simple;
  auto ex1_initial = ex1->ex_reduction_features.template get<VW980::simple_label_reduction_features>().initial;
  auto ex2_initial = ex2->ex_reduction_features.template get<VW980::simple_label_reduction_features>().initial;
...
API char* ComputeDiffDescriptionCbLabels(VW980::example* ex1, VW980::example* ex2)
{
  VW980::cb_label ld1 = ex1->l.cb;
  VW980::cb_label ld2 = ex2->l.cb;
  ...
  else
  {
    ...
    {
      VW980::cb_class c1 = ld1.costs[i];
      VW980::cb_class c2 = ld2.costs[i];
