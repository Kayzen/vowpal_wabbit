    ...
    VW980::experimental::api_status* status)
{
  ...
  try
  {
    ...
    {
      VW980::parsers::json::read_line_json<false>(
          *workspace->vw, examples, json, length, std::bind(get_example, example_pool_context));
    }

    VW980::setup_examples(*workspace->vw, examples);
    ...
    VW980::experimental::api_status* status)
{
  ...
  try
  {
    if (workspace->vw->audit)
    {
      VW980::parsers::json::read_line_decision_service_json<true>(*workspace->vw, examples, actual_json, length, copy_json,
          std::bind(get_example, example_pool_context), interaction);
    }
    else
    {
      VW980::parsers::json::read_line_decision_service_json<false>(*workspace->vw, examples, actual_json, length,
