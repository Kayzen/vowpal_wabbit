...
std::shared_ptr<std::vector<char>> get_cache_buffer(const std::string& es)
{
  ...
  vw->example_parser->output.add_file(VW980::io::create_vector_writer(buffer));
  auto* ae = &VW980::get_unused_example(vw.get());
  VW980::parsers::text::read_line(*vw, ae, const_cast<char*>(es.c_str()));

  VW980::parsers::cache::details::cache_temp_buffer temp_buf;
...
static void bench_cache_io_buf(benchmark::State& state, ExtraArgs&&... extra_args)
{
  ...
  auto vw = VW980::initialize(VW980::make_unique<VW980::config::options_cli>(std::vector<std::string>{"--cb", "2", "--quiet"}));
  io_buf io_buffer;
  io_buffer.add_file(VW980::io::create_buffer_view(cache_buffer->data(), cache_buffer->size()));
  VW980::multi_ex examples;
  examples.push_back(&VW980::get_unused_example(vw.get()));

  for (auto _ : state)
  {
    VW980::parsers::cache::read_example_from_cache(vw.get(), io_buffer, examples);
