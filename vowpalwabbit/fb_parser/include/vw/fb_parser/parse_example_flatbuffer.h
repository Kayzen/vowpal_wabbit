...
public:
  parser() = default;
  const VW980::parsers::flatbuffer::ExampleRoot* data();
  bool parse_examples(VW980::workspace* all, io_buf& buf, VW980::multi_ex& examples, uint8_t* buffer_pointer = nullptr);

private:
  const VW980::parsers::flatbuffer::ExampleRoot* _data;
  ...
  const VW980::parsers::flatbuffer::MultiExample* _multi_example_object = nullptr;
  uint32_t _labeled_action = 0;
  uint64_t _c_hash = 0;

  bool parse(io_buf& buf, uint8_t* buffer_pointer = nullptr);
  void process_collection_item(VW980::workspace* all, VW980::multi_ex& examples);
  void parse_example(VW980::workspace* all, example* ae, const Example* eg);
  void parse_multi_example(VW980::workspace* all, example* ae, const MultiExample* eg);
  void parse_namespaces(VW980::workspace* all, example* ae, const Namespace* ns);
  void parse_features(VW980::workspace* all, features& fs, const Feature* feature, const flatbuffers::String* ns);
  void parse_flat_label(shared_data* sd, example* ae, const Example* eg, VW980::io::logger& logger);
  ...
  void parse_mc_label(shared_data* sd, polylabel* l, const MultiClass* label, VW980::io::logger& logger);
