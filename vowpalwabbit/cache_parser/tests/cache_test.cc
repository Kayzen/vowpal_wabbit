...
{
  auto workspace = VW980::initialize(vwtest::make_args("--quiet"));
  VW980::example src_ex;
  VW980::parsers::text::read_line(*workspace, &src_ex, "3.5 |ns1 example value test |ss2 ex:0.5");

  auto backing_vector = std::make_shared<std::vector<char>>();
  VW980::io_buf io_writer;
  io_writer.add_file(VW980::io::create_vector_writer(backing_vector));

  VW980::parsers::cache::details::cache_temp_buffer temp_buffer;
  ...
  VW980::example dest_ex;
  examples.push_back(&dest_ex);
  VW980::parsers::cache::read_example_from_cache(workspace.get(), io_reader, examples);
...
TEST(Cache, WriteAndReadLargeExample)
{
  auto workspace = VW980::initialize(vwtest::make_args("--quiet"));
  VW980::example src_ex;
  VW980::parsers::text::read_line(*workspace, &src_ex,
...
TEST(Cache, WriteAndReadTag)
{
  ...
  VW980::parsers::cache::details::cache_tag(io_writer, tag);
  io_writer.flush();

  VW980::io_buf io_reader;
  io_reader.add_file(VW980::io::create_buffer_view(backing_vector->data(), backing_vector->size()));

  VW980::v_array<char> read_tag;
  VW980::parsers::cache::details::read_cached_tag(io_reader, read_tag);
...
TEST(Cache, WriteAndReadIndex)
{
  ...
  VW980::namespace_index index = 79;
  VW980::parsers::cache::details::cache_index(io_writer, index);
  io_writer.flush();

  VW980::io_buf io_reader;
  io_reader.add_file(VW980::io::create_buffer_view(backing_vector->data(), backing_vector->size()));

  VW980::namespace_index read_index = 0;
  VW980::parsers::cache::details::read_cached_index(io_reader, read_index);
...
TEST(Cache, WriteAndReadFeatures)
{
  ...
  VW980::parsers::cache::details::cache_features(io_writer, feats, mask);
  io_writer.flush();

  VW980::io_buf io_reader;
  io_reader.add_file(VW980::io::create_buffer_view(backing_vector->data(), backing_vector->size()));

  VW980::features read_feats;
  bool sorted = false;
  VW980::parsers::cache::details::read_cached_features(io_reader, read_feats, sorted);
