...
TEST(ParseJson, Cb)
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--cb_adf", "--json", "--chain_hash", "--no_stdin", "--quiet"));
  auto examples = vwtest::parse_json(*vw, json_text);
  ...
  std::string json_text = R"(
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--json", "--chain_hash", "--cats", "4", "--min_value=185",
  ...
  std::string json_text = R"(
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--json", "--chain_hash", "-t", "--cats", "4", "--min_value=185",
      "--max_value=23959", "--bandwidth", "1", "--no_stdin", "--quiet"));
  auto examples = vwtest::parse_json(*vw, json_text);
...
TEST(ParseJson, Ccb)
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--ccb_explore_adf", "--json", "--chain_hash", "--no_stdin", "--quiet"));

  auto examples = vwtest::parse_json(*vw, json_text);

  EXPECT_EQ(examples.size(), 8);
  EXPECT_EQ(examples[0]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SHARED);
  EXPECT_EQ(examples[1]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[2]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[3]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[4]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[5]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
  EXPECT_EQ(examples[6]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
  EXPECT_EQ(examples[7]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
...
TEST(ParseJson, CbAsCcb)
{
  ...
  EXPECT_EQ(examples[2]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[3]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[4]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
  ...
  std::string json_text = R"(
{
  ...
  auto slates_vw = VW980::initialize(
      vwtest::make_args("--slates", "--dsjson", "--chain_hash", "--no_stdin", "--noconstant", "--quiet"));
  auto examples = vwtest::parse_json(*slates_vw, json_text);

  EXPECT_EQ(examples.size(), 8);
  EXPECT_EQ(examples[0]->l.slates.type, VW980::slates::example_type::SHARED);
  EXPECT_EQ(examples[1]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[2]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[3]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[4]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[5]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[6]->l.slates.type, VW980::slates::example_type::SLOT);
  EXPECT_EQ(examples[7]->l.slates.type, VW980::slates::example_type::SLOT);
  ...
  VW980::finish_example(*slates_vw, examples);
}
...
TEST(ParseJson, TextDoesNotChangeInput)
{
  ...
  auto ccb_vw = VW980::initialize(vwtest::make_args("--ccb_explore_adf", "--dsjson", "--quiet"));

  VW980::multi_ex examples;
  examples.push_back(&VW980::get_unused_example(ccb_vw.get()));
  ccb_vw->example_parser->text_reader(
      ccb_vw.get(), VW980::string_view(json_text.c_str(), strlen(json_text.c_str())), examples);

  EXPECT_EQ(json_text, json_text_copy);

  VW980::multi_ex vec;
  for (const auto& ex : examples) { vec.push_back(ex); }
  VW980::finish_example(*ccb_vw, vec);
}
  ...
  const std::string json_deduped_text = R"(
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--json", "--chain_hash", "--cb_explore_adf", "--no_stdin", "--quiet"));

  std::unordered_map<uint64_t, VW980::example*> dedup_examples;
  VW980::multi_ex examples;
  ...
  VW980::parsers::json::read_line_json<true>(
      *vw, examples, (char*)action_1.c_str(), action_1.length(), std::bind(VW980::get_unused_example, vw.get()));
  ...
  examples.push_back(&VW980::get_unused_example(vw.get()));
  VW980::parsers::json::read_line_json<true>(
      *vw, examples, (char*)action_2.c_str(), action_2.length(), std::bind(VW980::get_unused_example, vw.get()));
  ...
  examples.push_back(&VW980::get_unused_example(vw.get()));
  VW980::parsers::json::read_line_json<true>(*vw, examples, (char*)json_deduped_text.c_str(), json_deduped_text.length(),
      std::bind(VW980::get_unused_example, vw.get()), &dedup_examples);
  ...
  for (auto* example : examples) { VW980::finish_example(*vw, *example); }
  for (auto& dedup : dedup_examples) { VW980::finish_example(*vw, *dedup.second); }
}
  ...
  const std::string json_deduped_text = R"(
{
  ...
  // parse json that includes dedup id's and re-use the examples from the dedup map instead of creating new ones
  examples.push_back(&VW980::get_unused_example(vw.get()));
  EXPECT_THROW(VW980::parsers::json::read_line_json<true>(*vw, examples, (char*)json_deduped_text.c_str(),
                   json_deduped_text.length(), std::bind(VW980::get_unused_example, vw.get()), &dedup_examples),
      VW980::vw_exception);
  ...
  const std::string json_deduped_text = R"(
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--json", "--chain_hash", "--ccb_explore_adf", "--no_stdin", "--quiet"));
  ...
  EXPECT_EQ(examples[1]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[2]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::ACTION);
  EXPECT_EQ(examples[3]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
  EXPECT_EQ(examples[4]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
  EXPECT_EQ(examples[5]->l.conditional_contextual_bandit.type, VW980::ccb_example_type::SLOT);
  ...
  const std::string json_deduped_text = R"(
{
  ...
  auto vw = VW980::initialize(vwtest::make_args("--json", "--chain_hash", "--slates", "--no_stdin", "--quiet"));
  ...
  EXPECT_EQ(examples[1]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[2]->l.slates.type, VW980::slates::example_type::ACTION);
  EXPECT_EQ(examples[3]->l.slates.type, VW980::slates::example_type::SLOT);
  EXPECT_EQ(examples[4]->l.slates.type, VW980::slates::example_type::SLOT);
...
TEST(ParseJson, SimpleVerifyExtents)
{
  ...
  EXPECT_EQ(examples[0]->feature_space['f'].namespace_extents[0],
      (VW980::namespace_extent{0, 4, VW980::hash_space(*vw, "features")}));
  EXPECT_EQ(examples[0]->feature_space['f'].namespace_extents[1],
      (VW980::namespace_extent{4, 5, VW980::hash_space(*vw, "features2")}));
