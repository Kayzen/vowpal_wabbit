...
using namespace VW980::continuous_actions;
using std::vector;
...
bool continuous_action_range_check(std::vector<pdf_seg> scores, float range_min, float range_max)
{
  ...
  float pdf_value = 0.f;
  uint64_t seed = 7791;
  auto scode = VW980::explore::sample_pdf(&seed, begin(scores), end(scores), chosen_value, pdf_value);
...
TEST(Explore, NewSamplePdf)
{
  ...
  const auto str_pdf = VW980::to_string(the_pdf);  // 2-3.5:0.0666667,3.5-4.5:0.8,4.5-6.2:0.0588235
  ...
  VW980::explore::sample_pdf(&seed, std::begin(the_pdf), std::end(the_pdf), chosen_action, pdf_value);
...
TEST(Explore, SampleContinuousActionStatistical)
{
  ...
  {
    const auto scode = VW980::explore::sample_pdf(&random_seed, begin(scores), end(scores), chosen_value, pdf_value);
    EXPECT_EQ(scode, S_EXPLORATION_OK);
...
TEST(Explore, SampleAfterNormalizingBasic)
{
  ...
  auto scode = VW980::explore::sample_after_normalizing(7791, begin(pdf), end(pdf), chosen_index);
  EXPECT_EQ(scode, S_EXPLORATION_OK);
  EXPECT_THAT(pdf, Pointwise(FloatNear(.0001f), expected));

  scode = VW980::explore::sample_after_normalizing(7791, begin(pdf), end(pdf), chosen_index);
  EXPECT_EQ(scode, S_EXPLORATION_OK);
  EXPECT_THAT(pdf, Pointwise(FloatNear(.0001f), expected));
...
TEST(Explore, SampleAfterNormalizingDegenerate)
{
  ...
  const auto scode = VW980::explore::sample_after_normalizing(7791, begin(pdf), end(pdf), chosen_index);
  EXPECT_EQ(scode, S_EXPLORATION_OK);
  EXPECT_THAT(pdf, Pointwise(FloatNear(.0001f), expected));
}
...
TEST(Explore, SwapTest)
{
  ...
  scode = VW980::explore::swap_chosen(begin(pdf), end(pdf), chosen_index);
...
TEST(Explore, EpsilonGreedy)
{
  ...
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::generate_epsilon_greedy(0.4f, 2, begin(pdf), end(pdf)));
...
{
  std::vector<float> pdf(4);
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::generate_epsilon_greedy(0.4f, 8, begin(pdf), end(pdf)));
...
TEST(Explore, EpsilonGreedyBadRange)
{
  ...
  float x;
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::generate_epsilon_greedy(0.4f, 0, begin(pdf), end(pdf)));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::generate_epsilon_greedy(0.4f, 0, &x, &x - 3));
...
TEST(Explore, Softmax)
{
  ...
  std::vector<float> pdf(4);
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::generate_softmax(0.2f, begin(scores), end(scores), begin(pdf), end(pdf)));
...
TEST(Explore, SoftmaxBadRange)
{
  ...
  float x;
  EXPECT_THAT(
      E_EXPLORATION_BAD_RANGE, VW980::explore::generate_softmax(0.2f, begin(scores), end(scores), begin(pdf), end(pdf)));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::generate_softmax(0.2f, begin(scores), end(scores), &x, &x - 3));
}
...
TEST(Explore, Bag)
{
  ...
  std::vector<float> pdf(4);
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::generate_bag(begin(top_actions), end(top_actions), begin(pdf), end(pdf)));
...
TEST(Explore, BagBadRange)
{
  ...
  EXPECT_THAT(
      E_EXPLORATION_BAD_RANGE, VW980::explore::generate_bag(begin(top_actions), end(top_actions), begin(pdf), end(pdf)));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::generate_bag(begin(top_actions), end(top_actions), &x, &x - 3));
}

TEST(Explore, EnforceMinimumProbability)
{
  std::vector<float> pdf = {1.f, 0, 0};
  VW980::explore::enforce_minimum_probability(0.3f, true, begin(pdf), end(pdf));
...
TEST(Explore, EnforceMinimumProbabilityNoZeros)
{
  ...
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(0.6f, false, begin(pdf), end(pdf)));
...
{
  std::vector<float> pdf = {0.f, 0.f, 0.f};
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(0.6f, true, begin(pdf), end(pdf)));
...
TEST(Explore, EnforceMinimumProbabilityEqualToAmt)
{
  ...
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(1.f, true, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNear(1e-3f), std::vector<float>{1.f / 3.f, 1.f / 3.f, 1.f / 3.f}));
}
...
{
  std::vector<float> pdf = {0.9f, 0.1f, 0};
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(1.f, false, begin(pdf), end(pdf)));
...
TEST(Explore, EnforceMinimumProbabilityBadRange)
{
  std::vector<float> pdf;
  float x;
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::enforce_minimum_probability(1.f, false, begin(pdf), end(pdf)));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::enforce_minimum_probability(1.f, false, &x, &x - 3));
}

TEST(Explore, EnforceMinimumProbabilityUniform1)
{
  std::vector<float> pdf = {0.9f, 0.1f, 0.f};
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(0.3f, true, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNear(1e-3f), std::vector<float>{.8f, .1f, .1f}));
}
...
{
  std::vector<float> pdf = {0.89f, 0.11f, 0.0f};
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(0.3f, false, begin(pdf), end(pdf)));
...
{
  std::vector<float> pdf = {0.89f, 0.11f, 0.0f};
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(0.0f, false, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNear(1e-3f), std::vector<float>{.89f, .11f, .0f}));

  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::enforce_minimum_probability(0.0f, true, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNear(1e-3f), std::vector<float>{.89f, .11f, .0f}));
}
...
TEST(Explore, Sampling)
{
  ...
  for (size_t i = 0; i < rep; i++)
  {
    std::stringstream s;
    s << "abcde" << i;
    ASSERT_EQ(0, VW980::explore::sample_after_normalizing(s.str().c_str(), std::begin(pdf), std::end(pdf), chosen_index));
...
TEST(Explore, SamplingRankBadRange)
{
  ...
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE,
      VW980::explore::sample_after_normalizing("abc", std::begin(pdf), std::end(pdf), chosen_index));
  EXPECT_THAT(E_EXPLORATION_BAD_RANGE, VW980::explore::sample_after_normalizing("abc", &x, &x - 3, chosen_index));
}
...
TEST(Explore, SamplingRankZeroPdf)
{
  ...
  EXPECT_THAT(
      S_EXPLORATION_OK, VW980::explore::sample_after_normalizing("abc", std::begin(pdf), std::end(pdf), chosen_index));
...
{
  VW980::action_scores probs;
  probs.push_back(VW980::action_score{1, 1.f});
  probs.push_back(VW980::action_score{2, 0.f});
  probs.push_back(VW980::action_score{3, 0.f});

  VW980::explore::mix_with_uniform(0.3f, begin_scores(probs), end_scores(probs));
...
TEST(Explore, MixWithUniformFloatsTest)
{
  ...
  EXPECT_THAT(S_EXPLORATION_OK, VW980::explore::mix_with_uniform(0.3f, begin(pdf), end(pdf)));
  EXPECT_THAT(pdf, Pointwise(FloatNear(1e-3f), std::vector<float>{.8f, .1f, .1f}));
}
...
{
  VW980::action_scores probs;
  probs.push_back(VW980::action_score{1, 0.9f});
  probs.push_back(VW980::action_score{2, 0.1f});
  probs.push_back(VW980::action_score{3, 0.f});
...
{
  VW980::action_scores probs;
  probs.push_back(VW980::action_score{1, 0.8f});
  probs.push_back(VW980::action_score{2, 0.2f});
  probs.push_back(VW980::action_score{3, 0.f});
