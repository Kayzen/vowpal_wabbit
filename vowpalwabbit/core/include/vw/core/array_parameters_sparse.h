...
using weight_map = std::unordered_map<uint64_t, std::shared_ptr<VW980::weight>>;
...
{
public:
  using iterator = details::sparse_iterator<VW980::weight>;
  using const_iterator = details::sparse_iterator<const VW980::weight>;
  ...
  VW980::weight* first() { THROW_OR_RETURN("Allreduce currently not supported in sparse", nullptr); }
  ...
  inline VW980::weight& operator[](size_t i) { return *(get_or_default_and_get(i)); }

  inline const VW980::weight& operator[](size_t i) const { return *(get_or_default_and_get(i)); }
...
private:
  ...
  std::function<void(VW980::weight*, uint64_t)> _default_func;

  // It is marked const so it can be used from both const and non const operator[]
  // The map itself is mutable to facilitate this
  VW980::weight* get_or_default_and_get(size_t i) const;
};
}  // namespace VW980
using sparse_parameters VW_DEPRECATED("sparse_parameters moved into VW namespace") = VW980::sparse_parameters;
