...
inline void dummy_func(DataT&, const VW980::audit_strings*)
{
...
void foreach_feature(WeightsT& /*weights*/, const VW980::features& fs, DataT& dat, uint64_t offset = 0, float mult = 1.)
{
...
inline void foreach_feature(WeightsT& weights, const VW980::features& fs, DataT& dat, uint64_t offset = 0, float mult = 1.)
{
  for (const auto& f : fs)
  {
    VW980::weight& w = weights[(f.index() + offset)];
...
inline void foreach_feature(
    const WeightsT& weights, const VW980::features& fs, DataT& dat, uint64_t offset = 0, float mult = 1.)
{
...
inline void generate_interactions(const std::vector<std::vector<VW980::namespace_index>>& interactions,
    ...
    VW980::details::generate_interactions_object_cache& cache)  // default value removed to eliminate
                                                             // ambiguity in old complers
{
  VW980::generate_interactions<DataT, WeightOrIndexT, FuncT, false, details::dummy_func<DataT>, WeightsT>(
...
inline void foreach_feature(WeightsT& weights, bool ignore_some_linear,
    std::array<bool, VW980::NUM_NAMESPACES>& ignore_linear,
    const std::vector<std::vector<VW980::namespace_index>>& interactions,
    const std::vector<std::vector<VW980::extent_term>>& extent_interactions, bool permutations, VW980::example_predict& ec,
    DataT& dat, size_t& num_interacted_features, VW980::details::generate_interactions_object_cache& cache)
{
  uint64_t offset = ec.ft_offset;
  if (ignore_some_linear)
  {
    for (VW980::example_predict::iterator i = ec.begin(); i != ec.end(); ++i)
    {
  ...
  {
    for (VW980::features& f : ec) { foreach_feature<DataT, FuncT, WeightsT>(weights, f, dat, offset); }
  }
...
inline void foreach_feature(WeightsT& weights, bool ignore_some_linear,
    ...
    const std::vector<std::vector<VW980::namespace_index>>& interactions,
    const std::vector<std::vector<VW980::extent_term>>& extent_interactions, bool permutations, VW980::example_predict& ec,
    DataT& dat, VW980::details::generate_interactions_object_cache& cache)
{
...
inline float inline_predict(WeightsT& weights, bool ignore_some_linear,
    ...
    const std::vector<std::vector<VW980::namespace_index>>& interactions,
    const std::vector<std::vector<VW980::extent_term>>& extent_interactions, bool permutations, VW980::example_predict& ec,
    VW980::details::generate_interactions_object_cache& cache, float initial = 0.f)
{
...
inline float inline_predict(WeightsT& weights, bool ignore_some_linear,
    ...
    const std::vector<std::vector<VW980::namespace_index>>& interactions,
    const std::vector<std::vector<VW980::extent_term>>& extent_interactions, bool permutations, VW980::example_predict& ec,
    size_t& num_interacted_features, VW980::details::generate_interactions_object_cache& cache, float initial = 0.f)
{
...
void foreach_feature(WeightsT& weights, const VW980::features& fs, DataT& dat, uint64_t offset = 0, float mult = 1.)
{
  VW980::foreach_feature<DataT, FuncT, WeightsT>(weights, fs, dat, offset, mult);
}
...
VW_DEPRECATED("Moved to VW namespace") inline void generate_interactions(
    ...
    VW980::details::generate_interactions_object_cache& cache)  // default value removed to eliminate
                                                             // ambiguity in old complers
{
  VW980::generate_interactions<DataT, WeightOrIndexT, FuncT, WeightsT>(
      interactions, extent_interactions, permutations, ec, dat, weights, num_interacted_features, cache);
}
...
inline void foreach_feature(WeightsT& weights, bool ignore_some_linear,
    ...
    DataT& dat, size_t& num_interacted_features, VW980::details::generate_interactions_object_cache& cache)
{
  VW980::foreach_feature<DataT, WeightOrIndexT, FuncT, WeightsT>(weights, ignore_some_linear, ignore_linear, interactions,
...
inline void foreach_feature(WeightsT& weights, bool ignore_some_linear,
    ...
    DataT& dat, VW980::details::generate_interactions_object_cache& cache)
{
  VW980::foreach_feature<DataT, WeightOrIndexT, FuncT, WeightsT>(
...
inline float inline_predict(WeightsT& weights, bool ignore_some_linear,
    ...
    size_t& num_interacted_features, VW980::details::generate_interactions_object_cache& cache, float initial = 0.f)
{
  return VW980::inline_predict(weights, ignore_some_linear, ignore_linear, interactions, extent_interactions, permutations,
