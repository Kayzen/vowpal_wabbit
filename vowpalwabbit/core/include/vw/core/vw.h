...
namespace VW980
{
using driver_output_func_t = VW980::trace_message_t;
...
VW980::workspace* initialize(std::unique_ptr<config::options_i, options_deleter_type> options, io_buf* model = nullptr,
    bool skip_model_load = false, trace_message_t trace_listener = nullptr, void* trace_context = nullptr);

// TODO: uncomment when all uses are migrated
VW_DEPRECATED("Replaced with new unique_ptr based overload.")
VW980::workspace* initialize(config::options_i& options, io_buf* model = nullptr, bool skip_model_load = false,
    trace_message_t trace_listener = nullptr, void* trace_context = nullptr);

// TODO: uncomment when all uses are migrated
VW_DEPRECATED("Replaced with new unique_ptr based overload.")
VW980::workspace* initialize(const std::string& s, io_buf* model = nullptr, bool skip_model_load = false,
    trace_message_t trace_listener = nullptr, void* trace_context = nullptr);

// TODO: uncomment when all uses are migrated
VW_DEPRECATED("Replaced with new unique_ptr based overload.")
VW980::workspace* initialize(int argc, char* argv[], io_buf* model = nullptr, bool skip_model_load = false,
    trace_message_t trace_listener = nullptr, void* trace_context = nullptr);

// TODO: uncomment when all uses are migrated
VW_DEPRECATED("Replaced with new unique_ptr based overload.")
VW980::workspace* seed_vw_model(VW980::workspace* vw_model, const std::string& extra_args,
    trace_message_t trace_listener = nullptr, void* trace_context = nullptr);
// Allows the input command line string to have spaces escaped by '\'

// TODO: uncomment when all uses are migrated
VW_DEPRECATED("Replaced with new unique_ptr based overload.")
VW980::workspace* initialize_escaped(std::string const& s, io_buf* model = nullptr, bool skip_model_load = false,
    trace_message_t trace_listener = nullptr, void* trace_context = nullptr);

// Experimental (VW980::setup_base_i):
VW_DEPRECATED("For scenarios requiring the builder, initialize_experimental should be used.")
VW980::workspace* initialize_with_builder(const std::string& s, io_buf* model = nullptr, bool skip_model_load = false,
    trace_message_t trace_listener = nullptr, void* trace_context = nullptr,
    std::unique_ptr<VW980::setup_base_i> = nullptr);
 ...
 * VW980::split_command_line() can be used to split the string similar to how a
 * shell would
 * \code
 * auto all = VW980::initialize(VW980::make_unique<VW980::config::options_cli>(
 *   VW980::split_command_line("--cb_explore_adf --epsilon=0.1 --quadratic=::")));
 * \endcode
 *
 * **Note:** You used to need to call VW980::finish() to free the workspace. This is no
 * longer needed and the destructor will free the workspace. However,
 * VW980::finish() would also do driver finalization steps, such as writing the output
 * model. This is not often needed in library mode but can be run using
 * VW980::workspace::finish().
 *
 * @param options The options to initialize the workspace with. Usually an
 * instance of VW980::config::options_cli.
 ...
 * @return std::unique_ptr<VW980::workspace> initialized workspace
 */
std::unique_ptr<VW980::workspace> initialize(std::unique_ptr<config::options_i> options,
    std::unique_ptr<VW980::io::reader> model_override_reader = nullptr, driver_output_func_t driver_output_func = nullptr,
    void* driver_output_func_context = nullptr, VW980::io::logger* custom_logger = nullptr);
...
std::unique_ptr<VW980::workspace> seed_vw_model(VW980::workspace& vw_model, const std::vector<std::string>& extra_args,
    driver_output_func_t driver_output_func = nullptr, void* driver_output_func_context = nullptr,
    VW980::io::logger* custom_logger = nullptr);
 ...
 * @return std::unique_ptr<VW980::workspace> initialized workspace
 */
std::unique_ptr<VW980::workspace> initialize_experimental(std::unique_ptr<config::options_i> options,
    std::unique_ptr<VW980::io::reader> model_override_reader = nullptr, driver_output_func_t driver_output_func = nullptr,
    void* driver_output_func_context = nullptr, VW980::io::logger* custom_logger = nullptr,
...
VW_DEPRECATED(
    "This functionality is now implemented by VW980::split_command_line which supports escaping, etc. This function will "
    "be removed in VW 10.")
...
const char* are_features_compatible(const VW980::workspace& vw1, const VW980::workspace& vw2);
...
VW_DEPRECATED(
    ...
    "VW980::workspace::finish().")
void finish(VW980::workspace& all, bool delete_all = true);

VW_WARNING_STATE_POP
void sync_stats(VW980::workspace& all);

void start_parser(VW980::workspace& all);
void end_parser(VW980::workspace& all);

VW_DEPRECATED(
    "It is no longer supported to query whether an example is a ring example. This function will be removed in VW 10")
bool is_ring_example(const VW980::workspace& all, const example* ae);
 ...
 */
example* read_example(VW980::workspace& all, const char* example_line);
example* read_example(VW980::workspace& all, const std::string& example_line);
...
example* import_example(VW980::workspace& all, const std::string& label, primitive_feature_space* features, size_t len);
...
void parse_example_label(VW980::workspace& all, example& ec, const std::string& label);
void setup_examples(VW980::workspace& all, VW980::multi_ex& examples);
void setup_example(VW980::workspace& all, example* ae);
example* new_unused_example(VW980::workspace& all);
...
feature* get_features(VW980::workspace& all, example* ec, size_t& feature_number);
void return_features(feature* f);

void add_constant_feature(const VW980::workspace& all, example* ec);
void add_label(example* ec, float label, float weight = 1, float base = 0);

// notify VW that you are done with the example.
void finish_example(VW980::workspace& all, example& ec);
void finish_example(VW980::workspace& all, multi_ex& ec);
void empty_example(VW980::workspace& all, example& ec);
...
primitive_feature_space* export_example(VW980::workspace& all, example* e, size_t& len);
void release_feature_space(primitive_feature_space* features, size_t len);
VW_DEPRECATED("VW980::releaseFeatureSpace renamed to VW980::release_feature_space")
inline void releaseFeatureSpace(primitive_feature_space* features, size_t len)  // NOLINT
{
  release_feature_space(features, len);
}

void save_predictor(VW980::workspace& all, const std::string& reg_name);
void save_predictor(VW980::workspace& all, io_buf& buf);

// inlines

// First create the hash of a namespace.
inline uint64_t hash_space(VW980::workspace& all, const std::string& s)
{
...
inline uint64_t hash_space_cstr(VW980::workspace& all, const char* fstr)
{
  return all.example_parser->hasher(fstr, strlen(fstr), all.hash_seed);
}
// Then use it as the seed for hashing features.
inline uint64_t hash_feature(VW980::workspace& all, const std::string& s, uint64_t u)
{
...
inline uint64_t hash_feature_cstr(VW980::workspace& all, const char* fstr, uint64_t u)
{
  return all.example_parser->hasher(fstr, strlen(fstr), u) & all.parse_mask;
}

inline uint64_t chain_hash(VW980::workspace& all, const std::string& name, const std::string& value, uint64_t u)
{
...
inline float get_weight(VW980::workspace& all, uint32_t index, uint32_t offset)
{
  return (&all.weights[static_cast<uint64_t>(index) << all.weights.stride_shift()])[offset];
}

inline void set_weight(VW980::workspace& all, uint32_t index, uint32_t offset, float value)
{
  (&all.weights[static_cast<uint64_t>(index) << all.weights.stride_shift()])[offset] = value;
}

inline uint32_t num_weights(VW980::workspace& all) { return static_cast<uint32_t>(all.length()); }

inline uint32_t get_stride(VW980::workspace& all) { return all.weights.stride(); }
