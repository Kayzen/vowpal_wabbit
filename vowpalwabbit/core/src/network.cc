...
std::unique_ptr<VW980::io::socket> VW980::details::open_vw_binary_socket(
    const std::string& host_and_optional_port, VW980::io::logger& logger)
{
  auto colon_pos = host_and_optional_port.find(':');
  if (colon_pos == std::string::npos)
  {
    return VW980::details::open_vw_binary_socket(host_and_optional_port, DEFAULT_PORT, logger);
  }

  std::string port = host_and_optional_port.substr(colon_pos + 1);
  std::string host = host_and_optional_port.substr(0, colon_pos);
  auto port_num = VW980::details::int_of_string(port, logger);

  return open_vw_binary_socket(host, VW980::cast_signed_to_unsigned<uint16_t>(port_num), logger);
}

std::unique_ptr<VW980::io::socket> NO_SANITIZE_UNDEFINED VW980::details::open_vw_binary_socket(
    const std::string& host, uint16_t port, VW980::io::logger& /* logger */)
{
  ...
  auto socket_resource = VW980::io::wrap_socket_descriptor(sd);
