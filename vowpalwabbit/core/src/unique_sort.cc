...
void VW980::unique_features(features& fs, int max)
{
  ...
  auto flat_extents = VW980::details::flatten_namespace_extents(fs.namespace_extents, fs.indices.size());
  ...
  fs.namespace_extents = VW980::details::unflatten_namespace_extents(flat_extents);
  ++last_index;
  fs.truncate_to(last_index);
}

void VW980::unique_sort_features(uint64_t parse_mask, VW980::example& ae)
{
  for (features& fs : ae)
  {
