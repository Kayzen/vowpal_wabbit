...
void VW980::reductions::slates_data::learn_or_predict(VW980::LEARNER::learner& base, multi_ex& examples)
{
  ...
  const size_t num_slots = std::count_if(examples.begin(), examples.end(),
      [](const example* example) { return example->l.slates.type == VW980::slates::example_type::SLOT; });
  ...
  for (size_t i = 0; i < examples.size(); i++)
  {
    ...
    else if (slates_label.type == slates::example_type::SLOT)
    {
      ...
      {
        ccb_label.outcome = new VW980::ccb_outcome();
  ...
  VW980::LEARNER::multiline_learn_or_predict<is_learn>(base, examples, examples[0]->ft_offset);
...
void VW980::reductions::slates_data::learn(VW980::LEARNER::learner& base, multi_ex& examples)
{
  learn_or_predict<true>(base, examples);
}

void VW980::reductions::slates_data::predict(VW980::LEARNER::learner& base, multi_ex& examples)
{
...
std::string VW980::reductions::generate_slates_label_printout(const std::vector<example*>& slots)
{
  size_t counter = 0;
...
float get_estimate(const VW980::action_scores& label_probs, float cost, const VW980::decision_scores_t& prediction_probs)
{
...
void update_stats_slates(const VW980::workspace& /* all */, VW980::shared_data& sd,
    const VW980::reductions::slates_data& /* data */, const VW980::multi_ex& ec_seq, VW980::io::logger& /* logger */)
{
  VW980::multi_ex slots;
  size_t num_features = 0;
  float loss = 0.;
  bool is_labeled = ec_seq[VW980::details::SHARED_EX_INDEX]->l.slates.labeled;
  float cost = is_labeled ? ec_seq[VW980::details::SHARED_EX_INDEX]->l.slates.cost : 0.f;
  VW980::v_array<VW980::action_score> label_probs;

  for (auto* ec : ec_seq)
  {
    num_features += ec->get_num_features();

    if (ec->l.slates.type == VW980::slates::example_type::SLOT)
    {
      slots.push_back(ec);
      if (is_labeled)
      {
  ...
  sd.update(holdout_example, is_labeled, loss, ec_seq[VW980::details::SHARED_EX_INDEX]->weight, num_features);
}

void output_example_prediction_slates(VW980::workspace& all, const VW980::reductions::slates_data& /* data */,
...
void print_update_slates(VW980::workspace& all, VW980::shared_data& /* sd */, const VW980::reductions::slates_data& /* data */,
    const VW980::multi_ex& ec_seq, VW980::io::logger& /* unused */)
{
  ...
  for (auto* ec : ec_seq)
  {
    ...
    if (ec->l.slates.type == VW980::slates::example_type::SLOT) { slots.push_back(ec); }
  }

  VW980::print_update_slates(all, slots, predictions, num_features);
}

void cleanup_example_slates(VW980::reductions::slates_data& /* data */, VW980::multi_ex& ec_seq)
{
...
template <bool is_learn>
void learn_or_predict(VW980::reductions::slates_data& data, VW980::LEARNER::learner& base, VW980::multi_ex& examples)
{
...
std::shared_ptr<VW980::LEARNER::learner> VW980::reductions::slates_setup(VW980::setup_base_i& stack_builder)
{
  options_i& options = *stack_builder.get_options();
  auto data = VW980::make_unique<slates_data>();
  ...
  auto l = make_reduction_learner(std::move(data), base, learn_or_predict<true>, learn_or_predict<false>,
      stack_builder.get_setupfn_name(slates_setup))
               ...
               .set_input_prediction_type(VW980::prediction_type_t::DECISION_PROBS)
               .set_output_prediction_type(VW980::prediction_type_t::DECISION_PROBS)
               .set_input_label_type(VW980::label_type_t::SLATES)
               .set_output_label_type(VW980::label_type_t::CCB)
