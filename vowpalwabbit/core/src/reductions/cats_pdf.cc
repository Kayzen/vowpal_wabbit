...
// Pass through
int cats_pdf::predict(VW980::example& ec, VW980::experimental::api_status*)
{
  VW_DBG(ec) << "cats_pdf::predict(), " << VW980::debug::features_to_string(ec) << endl;
  _base->predict(ec);
  return VW980::experimental::error_code::success;
}

// Pass through
int cats_pdf::learn(VW980::example& ec, VW980::experimental::api_status*)
{
  assert(!ec.test_only);
  VW_DBG(ec) << "cats_pdf::learn(), " << VW980::to_string(ec.l.cb_cont) << VW980::debug::features_to_string(ec) << endl;
...
void predict_or_learn(cats_pdf& reduction, learner&, VW980::example& ec)
{
...
void update_stats_cats_pdf(const VW980::workspace& /* all */, VW980::shared_data& sd, const cats_pdf& /* data */,
...
void output_example_prediction_cats_pdf(
    VW980::workspace& all, const cats_pdf& /* data */, const VW980::example& ec, VW980::io::logger& /* unused */)
{
  // output to the prediction to all files
  const auto str = VW980::to_string(ec.pred.pdf, VW980::details::AS_MANY_AS_NEEDED_FLOAT_FORMATTING_DECIMAL_PRECISION);
...
void print_update_cats_pdf(VW980::workspace& all, VW980::shared_data& /* sd */, const cats_pdf& /* data */,
    const VW980::example& ec, VW980::io::logger& /* unused */)
{
  ...
  if (should_print_driver_update)
  {
    all.sd->print_update(*all.trace_message, all.holdout_set_off, all.current_pass,
        ...
        VW980::to_string(ec.pred.pdf, VW980::details::DEFAULT_FLOAT_FORMATTING_DECIMAL_PRECISION),  // Prediction
        ec.get_num_features());
  }
}
}  // namespace
...
std::shared_ptr<VW980::LEARNER::learner> VW980::reductions::cats_pdf_setup(setup_base_i& stack_builder)
{
  ...
  auto p_reduction = VW980::make_unique<cats_pdf>(require_singleline(p_base).get(), always_predict);
