...
{
public:
  scorer(VW980::workspace* all) : all(all) {}
  VW980::workspace* all;
};  // for set_minmax, loss

template <bool is_learn, float (*link)(float in)>
void predict_or_learn(scorer& s, VW980::LEARNER::learner& base, VW980::example& ec)
{
...
inline void multipredict(scorer& /*unused*/, VW980::LEARNER::learner& base, VW980::example& ec, size_t count,
    size_t /*unused*/, VW980::polyprediction* pred, bool finalize_predictions)
{
...
void update(scorer& s, VW980::LEARNER::learner& base, VW980::example& ec)
{
...
// y = f(x) -> [0, 1]
inline float logistic(float in) { return 1.f / (1.f + VW980::details::correctedExp(-in)); }
...
//    y = f(x) -> [-1, 1]
inline float glf1(float in) { return 2.f / (1.f + VW980::details::correctedExp(-in)) - 1.f; }

inline float id(float in) { return in; }
}  // namespace

std::shared_ptr<VW980::LEARNER::learner> VW980::reductions::scorer_setup(VW980::setup_base_i& stack_builder)
{
  ...
  using predict_or_learn_fn_t = void (*)(scorer&, VW980::LEARNER::learner&, VW980::example&);
  using multipredict_fn_t =
      void (*)(scorer&, VW980::LEARNER::learner&, VW980::example&, size_t, size_t, VW980::polyprediction*, bool);
  ...
  auto s = VW980::make_unique<scorer>(&all);
