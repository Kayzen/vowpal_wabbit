...
public:
  ...
  void init(learner* p_base, std::shared_ptr<VW980::rand_state> random_state);

private:
  std::shared_ptr<VW980::rand_state> _p_random_state;
  VW980::continuous_actions::probability_density_function _pred_pdf;
  learner* _base = nullptr;
};

int sample_pdf::learn(VW980::example& ec, VW980::experimental::api_status*)
{
  // one of the base reductions will call predict so we need a valid
  // predict buffer
  _pred_pdf.clear();
  {  // scope to predict & restore prediction
    auto restore = VW980::swap_guard(ec.pred.pdf, _pred_pdf);
    _base->learn(ec);
  }
  return VW980::experimental::error_code::success;
}

int sample_pdf::predict(VW980::example& ec, VW980::experimental::api_status*)
{
  _pred_pdf.clear();
  ...
  const int ret_code = VW980::explore::sample_pdf(
  ...
  if (ret_code != S_EXPLORATION_OK) { return VW980::experimental::error_code::sample_pdf_failed; }

  return VW980::experimental::error_code::success;
}

void sample_pdf::init(learner* p_base, std::shared_ptr<VW980::rand_state> random_state)
{
  _base = p_base;
...
void predict_or_learn(sample_pdf& reduction, learner&, VW980::example& ec)
{
  VW980::experimental::api_status status;
  if (is_learn) { reduction.learn(ec, &status); }
  else
  {
    if (VW980::experimental::error_code::success != reduction.predict(ec, &status))
      THROW(VW980::experimental::error_code::sample_pdf_failed_s);
  }
...
std::shared_ptr<VW980::LEARNER::learner> VW980::reductions::sample_pdf_setup(VW980::setup_base_i& stack_builder)
{
  ...
  auto p_reduction = VW980::make_unique<sample_pdf>();
