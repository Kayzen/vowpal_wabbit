...
public:
  cb_explore_adf_synthcover(float epsilon, float psi, size_t synthcoversize,
      std::shared_ptr<VW980::rand_state> random_state, VW980::version_struct model_file_version);
  ...
  void save_load(VW980::io_buf& model_file, bool read, bool text);
...
template <bool is_learn>
void cb_explore_adf_synthcover::predict_or_learn_impl(VW980::LEARNER::learner& base, VW980::multi_ex& examples)
{
  ...
  if (it != examples.end())
  {
    const VW980::cb_class logged = (*it)->l.cb.costs[0];
  ...
  for (size_t i = 0; i < num_actions; i++) { preds[i].score = VW980::math::clamp(preds[i].score, _min_cost, _max_cost); }
  std::make_heap(preds.begin(), preds.end(), std::greater<VW980::action_score>());
  ...
  {
    std::pop_heap(preds.begin(), preds.end(), std::greater<VW980::action_score>());
    ...
    std::push_heap(preds.begin(), preds.end(), std::greater<VW980::action_score>());
  }

  VW980::explore::enforce_minimum_probability(_epsilon, true, begin_scores(_action_probs), end_scores(_action_probs));

  std::sort(_action_probs.begin(), _action_probs.end(), std::greater<VW980::action_score>());

  for (size_t i = 0; i < num_actions; i++) { preds[i] = _action_probs[i]; }
}

void cb_explore_adf_synthcover::save_load(VW980::io_buf& model_file, bool read, bool text)
{
...
std::shared_ptr<VW980::LEARNER::learner> VW980::reductions::cb_explore_adf_synthcover_setup(VW980::setup_base_i& stack_builder)
{
  ...
  auto data = VW980::make_unique<explore_type>(all.global_metrics.are_metrics_enabled(), epsilon, psi,
      VW980::cast_to_smaller_type<size_t>(synthcoversize), all.get_random_state(), all.model_file_ver);
