...
void VW980::details::print_scalars(
...
{
public:
  std::array<bool, VW980::NUM_NAMESPACES> namespaces{};  // the set of namespaces to evaluate.
  std::vector<policy_data> evals;                     // accrued losses of features.
  std::pair<bool, VW980::cb_class> optional_observation;
  VW980::v_array<uint64_t> policies;
  double total = 0.;
  uint32_t num_classes = 0;
  bool learn = false;

  VW980::v_array<VW980::namespace_index> indices;  // excluded namespaces
  std::array<VW980::features, VW980::NUM_NAMESPACES> feature_space;
...
void predict_or_learn(mwt& c, learner& base, VW980::example& ec)
{
  ...
  if (c.optional_observation.first)
  {
    ...
    {
      if (c.namespaces[ns]) { VW980::foreach_feature<mwt, value_policy>(c.all, ec.feature_space[ns], c); }
    }
    ...
    for (unsigned char ns : ec.indices)
    {
      if (c.namespaces[ns])
      {
        ...
        if (learn)
        {
          ...
          for (VW980::features::iterator& f : ec.feature_space[ns])
          {
            uint64_t new_index =
  ...
  VW980::v_array<float> preds = ec.pred.scalars;
...
void update_stats_mwt(const VW980::workspace& /* all */, VW980::shared_data& sd, const mwt& data, const VW980::example& ec,
...
void output_example_prediction_mwt(
    VW980::workspace& all, const mwt& /* data */, const VW980::example& ec, VW980::io::logger& /* unused */)
{
  for (auto& sink : all.final_prediction_sink)
  {
    VW980::details::print_scalars(sink.get(), ec.pred.scalars, ec.tag, all.logger);
  }
}

void print_update_mwt(
    VW980::workspace& all, VW980::shared_data& /* sd */, const mwt& data, const VW980::example& ec, VW980::io::logger& /* unused */)
{
...
void save_load(mwt& c, VW980::io_buf& model_file, bool read, bool text)
{
  ...
  {
    msg << "policies: ";
    for (VW980::feature_index& policy : c.policies) { msg << policy << " "; }
  }

  VW980::details::bin_text_read_write_fixed_validated(model_file, reinterpret_cast<char*>(c.policies.begin()),
      policies_size * sizeof(VW980::feature_index), read, msg, text);

  // c.evals is already initialized nicely to the same size as the regressor.
  for (VW980::feature_index& policy : c.policies)
  {
...
std::shared_ptr<VW980::LEARNER::learner> VW980::reductions::mwt_setup(VW980::setup_base_i& stack_builder)
{
  options_i& options = *stack_builder.get_options();
  VW980::workspace& all = *stack_builder.get_all_pointer();
  auto c = VW980::make_unique<mwt>();
  std::string s;
  ...
  void (*learn_ptr)(mwt&, learner&, VW980::example&) = nullptr;
  void (*pred_ptr)(mwt&, learner&, VW980::example&) = nullptr;
  ...
  auto l = make_reduction_learner(std::move(c), require_singleline(base), learn_ptr, pred_ptr,
      stack_builder.get_setupfn_name(mwt_setup) + name_addition)
               ...
               .set_output_prediction_type(VW980::prediction_type_t::SCALARS)
