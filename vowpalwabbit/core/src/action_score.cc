...
void VW980::details::print_action_score(
    VW980::io::writer* f, const VW980::v_array<action_score>& a_s, const VW980::v_array<char>& tag, VW980::io::logger& logger)
{
  if (f == nullptr) { return; }

  std::stringstream ss;
  ss << VW980::to_string(a_s);
...
std::ostream& VW980::operator<<(std::ostream& os, const action_score& a_s)
{
  os << "(" << a_s.action << "," << a_s.score << ")";
  return os;
}

std::string VW980::to_string(const action_scores& action_scores_or_probs, int decimal_precision)
{
  std::ostringstream ss;
  std::string delim;
  for (const auto& item : action_scores_or_probs)
  {
    ss << delim << fmt::format("{}:{}", item.action, VW980::fmt_float(item.score, decimal_precision));
    delim = ",";
  }
  return ss.str();
}

size_t VW980::model_utils::read_model_field(io_buf& io, action_score& a_s)
{
  size_t bytes = 0;
