...
using callback_map = typename std::map<size_t, std::function<bool(cb_sim&, VW980::workspace&, VW980::multi_ex&)>>;
...
public:
  ...
  std::pair<std::string, float> get_action(VW980::workspace* vw, const std::map<std::string, std::string>& context);
  const std::string& choose_user();
  const std::string& choose_time_of_day();
  std::vector<float> run_simulation(VW980::workspace* vw, size_t num_iterations, bool do_learn = true, size_t shift = 1,
      const std::vector<uint64_t>& swap_after = std::vector<uint64_t>());
  std::vector<float> run_simulation_hook(VW980::workspace* vw, size_t num_iterations, callback_map& callbacks,
...
private:
  void call_if_exists(VW980::workspace& vw, VW980::multi_ex& ex, const callback_map& callbacks, const size_t event);
};
