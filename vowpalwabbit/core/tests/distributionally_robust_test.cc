...
TEST(DistributionallyRobust, InverseChisq)
{
  ...
  {
    double v = VW980::estimators::chi_squared::chisq_onedof_isf(pt.first);
...
TEST(DistributionallyRobust, RecomputeDuals)
{
  ...
  VW980::details::Duals duals[] = {{true, 0, 0, 0, 1}, {false, 0.186284935714629, -0.5242563567278763, 0, 1.999},
  ...
  auto onlinechisq = VW980::make_unique<VW980::estimators::chi_squared>(0.05, 0.999);
