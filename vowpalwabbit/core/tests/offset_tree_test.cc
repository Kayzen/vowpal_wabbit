...
namespace std
{
std::ostream& operator<<(std::ostream& os, const VW980::reductions::offset_tree::tree_node& node)
{
...
public:
  ...
  void test_predict(VW980::example& ec)
  {
    ...
    ec.pred.a_s.push_back(VW980::action_score{0, curr_pred.first});
    ec.pred.a_s.push_back(VW980::action_score{1, curr_pred.second});
  }
...
std::shared_ptr<learner> get_test_harness(const predictions_t& bottom_learner_predictions)
{
  ...
  auto test_learner = VW980::LEARNER::make_bottom_learner(
      ...
      "test_learner", VW980::prediction_type_t::ACTION_SCORES, VW980::label_type_t::CB)
...
void predict_test_helper(const predictions_t& bottom_learner_predictions, const scores_t& expected_scores)
{
  ...
  VW980::reductions::offset_tree::offset_tree tree(static_cast<uint32_t>(expected_scores.size()));
  tree.init();
  VW980::example ec;
...
TEST(OffsetTree, OffsetTreeLearnBasic)
{
  ...
  VW980::reductions::offset_tree::offset_tree tree(3);
  tree.init();
  VW980::example ec;
  ec.l.cb.costs.push_back(VW980::cb_class{-1.0f, 1, 0.5f});
...
{
  VW980::reductions::offset_tree::min_depth_binary_tree tree;
  tree.build_tree(11);
  std::vector<VW980::reductions::offset_tree::tree_node> expected = {{0, 0, 0, 11, true}, {1, 0, 0, 11, true},
...
TEST(OffsetTree, BuildMinDepthTree4)
{
  VW980::reductions::offset_tree::min_depth_binary_tree tree;
  tree.build_tree(4);
  std::vector<VW980::reductions::offset_tree::tree_node> expected = {{0, 0, 0, 4, true}, {1, 0, 0, 4, true},
...
TEST(OffsetTree, BuildMinDepthTree3)
{
  VW980::reductions::offset_tree::min_depth_binary_tree tree;
  tree.build_tree(3);
  std::vector<VW980::reductions::offset_tree::tree_node> expected = {
...
TEST(OffsetTree, BuildMinDepthTree1)
{
  VW980::reductions::offset_tree::min_depth_binary_tree tree;
  tree.build_tree(1);
  std::vector<VW980::reductions::offset_tree::tree_node> expected = {{0, 0, 0, 0, true}};
  EXPECT_THAT(tree.nodes, ContainerEq(expected));
}
