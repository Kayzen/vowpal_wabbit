...
namespace std
{
std::ostream& operator<<(std::ostream& os, const std::pair<VW980::namespace_index, uint64_t>& obj)
{
...
template <VW980::generate_func_t<VW980::namespace_index> generate_func, bool leave_duplicate_interactions>
void eval_count_of_generated_ft_naive(
    VW980::workspace& all, VW980::example_predict& ec, size_t& new_features_cnt, float& new_features_value)
{
  ...
  auto interactions = VW980::details::compile_interactions<generate_func, leave_duplicate_interactions>(
      all.interactions, std::set<VW980::namespace_index>(ec.indices.begin(), ec.indices.end()));

  VW980::v_array<float> results;

  eval_gen_data dat(new_features_cnt, new_features_value);
  size_t ignored = 0;
  ec.interactions = &interactions;
  VW980::generate_interactions<eval_gen_data, uint64_t, ft_cnt, false, nullptr>(all, ec, dat, ignored);
  ec.interactions = &all.interactions;
}

template <VW980::generate_func_t<VW980::extent_term> generate_func, bool leave_duplicate_interactions>
void eval_count_of_generated_ft_naive(
    VW980::workspace& all, VW980::example_predict& ec, size_t& new_features_cnt, float& new_features_value)
{
  // Only makes sense to do this when not in permutations mode.
  ...
  new_features_cnt = 0;
  new_features_value = 0.;
  std::set<VW980::extent_term> seen_extents;
  ...
  auto interactions = VW980::details::compile_extent_interactions<generate_func, leave_duplicate_interactions>(
...
{
  auto vw = VW980::initialize(vwtest::make_args("--quiet", "-q", "::", "--noconstant"));
  auto* ex = VW980::read_example(*vw, "3 |f a b c |e x y z");

  size_t naive_features_count;
  float naive_features_value;
  eval_count_of_generated_ft_naive<VW980::details::generate_namespace_combinations_with_repetition<VW980::namespace_index>,
      false>(*vw, *ex, naive_features_count, naive_features_value);

  auto interactions =
      VW980::details::compile_interactions<VW980::details::generate_namespace_combinations_with_repetition, false>(
          vw->interactions, std::set<VW980::namespace_index>(ex->indices.begin(), ex->indices.end()));
  ex->interactions = &interactions;
  ex->extent_interactions = &vw->extent_interactions;
  float fast_features_value = VW980::eval_sum_ft_squared_of_generated_ft(
...
{
  auto vw = VW980::initialize(vwtest::make_args(
      "--quiet", "--experimental_full_name_interactions", "fff|eee|gg", "ggg|gg", "gg|gg|ggg", "--noconstant"));
  auto* ex = VW980::read_example(*vw, "3 |fff a b c |eee x y z |ggg a b |gg c d");

  size_t naive_features_count;
  float naive_features_value;
  eval_count_of_generated_ft_naive<VW980::details::generate_namespace_combinations_with_repetition<VW980::extent_term>,
      false>(*vw, *ex, naive_features_count, naive_features_value);
...
{
  auto vw = VW980::initialize(vwtest::make_args("--quiet", "-permutations", "--experimental_full_name_interactions",
...
// {
//   std::set<VW980::namespace_index> namespaces = {'a', 'b'};
//   auto result = VW980::details::generate_namespace_combinations_with_repetition(namespaces, 2);

//   std::vector<std::vector<VW980::namespace_index>> compare_set = {{'b', 'a'}, {'a', 'a'}, {'b', 'b'}};
...
{
  std::set<VW980::namespace_index> namespaces = {'a', 'b'};
  auto result = VW980::details::generate_namespace_permutations_with_repetition(namespaces, 2);

  std::vector<std::vector<VW980::namespace_index>> compare_set = {{'b', 'a'}, {'a', 'b'}, {'a', 'a'}, {'b', 'b'}};
...
{
  std::vector<std::vector<VW980::namespace_index>> input = {{'b', 'a'}, {'a', 'b', 'a'}, {'a', 'a'}, {'b', 'b'}};

  size_t removed_count = 0;
  size_t sorted_count = 0;
  VW980::details::sort_and_filter_duplicate_interactions(input, false, removed_count, sorted_count);

  std::vector<std::vector<VW980::namespace_index>> compare_set = {{'b', 'a'}, {'a', 'a', 'b'}, {'a', 'a'}, {'b', 'b'}};
...
{
  std::set<VW980::namespace_index> indices = {'a', 'b', 'c', 'd'};
  std::vector<std::vector<VW980::namespace_index>> interactions = {{':', 'a'}};

  // Permutations implies leave duplicate interactions (second template arg)
  auto result_perms =
      VW980::details::compile_interactions<VW980::details::generate_namespace_permutations_with_repetition, true>(
          interactions, indices);
  auto result_combs =
      VW980::details::compile_interactions<VW980::details::generate_namespace_combinations_with_repetition, false>(
          interactions, indices);

  std::vector<std::vector<VW980::namespace_index>> compare_set = {{'a', 'a'}, {'b', 'a'}, {'c', 'a'}, {'d', 'a'}};
...
TEST(Interactions, CompileInteractionsQuadraticCombinations)
{
  ...
  auto result = VW980::details::compile_interactions<VW980::details::generate_namespace_combinations_with_repetition, false>(
      interactions, indices);

  std::vector<std::vector<VW980::namespace_index>> compare_set = {{'a', 'a'}, {'a', 'b'}, {'a', 'c'}, {'a', 'd'},
...
TEST(Interactions, CompileInteractionsQuadraticPermutations)
{
  ...
  auto result = VW980::details::compile_interactions<VW980::details::generate_namespace_permutations_with_repetition, true>(
      interactions, indices);
...
TEST(Interactions, CompileInteractionsCubicCombinations)
{
  ...
  std::vector<std::vector<VW980::namespace_index>> compare_set = {
      {'a', 'a', 'a'},
      {'a', 'a', 'b'},
      {'a', 'a', 'c'},
      {'a', 'a', 'd'},
...
TEST(Interactions, ParseFullNameInteractionsTest)
{
  ...
  {
    auto a = VW980::details::parse_full_name_interactions(*vw, "a|b");
    std::vector<VW980::extent_term> expected = {
        VW980::extent_term{'a', VW980::hash_space(*vw, "a")}, VW980::extent_term{'b', VW980::hash_space(*vw, "b")}};
    EXPECT_THAT(a, ContainerEq(expected));
  }

  {
    auto a = VW980::details::parse_full_name_interactions(*vw, "art|bat|and");
    std::vector<VW980::extent_term> expected = {VW980::extent_term{'a', VW980::hash_space(*vw, "art")},
        VW980::extent_term{'b', VW980::hash_space(*vw, "bat")}, VW980::extent_term{'a', VW980::hash_space(*vw, "and")}};
    EXPECT_THAT(a, ContainerEq(expected));
  }

  {
    auto a = VW980::details::parse_full_name_interactions(*vw, "art|:|and");
    std::vector<VW980::extent_term> expected = {VW980::extent_term{'a', VW980::hash_space(*vw, "art")},
        VW980::extent_term{VW980::details::WILDCARD_NAMESPACE, VW980::details::WILDCARD_NAMESPACE},
        VW980::extent_term{'a', VW980::hash_space(*vw, "and")}};
    EXPECT_THAT(a, ContainerEq(expected));
  }

  EXPECT_THROW(VW980::details::parse_full_name_interactions(*vw, "||"), VW980::vw_exception);
  EXPECT_THROW(VW980::details::parse_full_name_interactions(*vw, "||||"), VW980::vw_exception);
  EXPECT_THROW(VW980::details::parse_full_name_interactions(*vw, "|a|||b"), VW980::vw_exception);
  EXPECT_THROW(VW980::details::parse_full_name_interactions(*vw, "abc|::"), VW980::vw_exception);
}

TEST(Interactions, ExtentVsCharInteractions)
{
  auto vw_char_inter = VW980::initialize(vwtest::make_args("--quiet", "-q", "AB"));
  auto vw_extent_inter =
      VW980::initialize(vwtest::make_args("--quiet", "--experimental_full_name_interactions", "group1|group2"));

  auto parse_and_return_num_fts = [&](const char* char_inter_example,
                                      const char* extent_inter_example) -> std::pair<size_t, size_t>
  {
    auto* ex_char = VW980::read_example(*vw_char_inter, char_inter_example);
    auto* ex_extent = VW980::read_example(*vw_extent_inter, extent_inter_example);
...
TEST(Interactions, ExtentInteractionExpansionTest)
{
  ...
  auto cleanup = VW980::scope_exit([&]() { VW980::finish_example(*vw, *ex); });

  VW980::details::generate_interactions_object_cache cache;

  {
    const auto extent_terms = VW980::details::parse_full_name_interactions(*vw, "user_info|user_info");
    size_t counter = 0;
    VW980::details::generate_generic_extent_combination_iterative(
  ...
  {
    const auto extent_terms = VW980::details::parse_full_name_interactions(*vw, "user_info|user_info|user_info");
    size_t counter = 0;
    VW980::details::generate_generic_extent_combination_iterative(
        ex->feature_space, extent_terms,
        [&](const std::vector<VW980::details::features_range_t>& combination)
  ...
  {
    const auto extent_terms = VW980::details::parse_full_name_interactions(*vw, "user_info|extra");
    size_t counter = 0;
    VW980::details::generate_generic_extent_combination_iterative(
        ex->feature_space, extent_terms,
        [&](const std::vector<VW980::details::features_range_t>& combination)
...
void do_interaction_feature_count_test(bool add_quadratic, bool add_cubic, bool combinations, bool no_constant)
{
  ...
  auto vw_char_inter = VW980::initialize(VW980::make_unique<VW980::config::options_cli>(char_cmd_line));
  auto vw_extent_inter = VW980::initialize(VW980::make_unique<VW980::config::options_cli>(extent_cmd_line));
  auto parse_and_return_num_fts = [&](const char* char_inter_example,
                                      const char* extent_inter_example) -> std::pair<size_t, size_t>
  {
    auto* ex_char = VW980::read_example(*vw_char_inter, char_inter_example);
