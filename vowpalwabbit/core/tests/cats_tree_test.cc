...
namespace std
{
std::ostream& operator<<(std::ostream& os, const VW980::reductions::cats::tree_node& node)
{
...
namespace VW980
{
std::ostream& operator<<(std::ostream& o, VW980::simple_label const& lbl)
{
  o << "{l=" << lbl.label << "}";
  return o;
}

std::ostream& operator<<(std::ostream& o, VW980::simple_label_reduction_features const& red_fts)
{
...
public:
  ...
  void test_predict(VW980::example& ec) { ec.pred.scalar = predictions[curr_idx++]; }
  ...
  vector<VW980::simple_label> labels;
...
std::shared_ptr<learner> get_test_harness(const predictions_t& bottom_learner_predictions, T*& pharness)
{
  ...
  auto test_harness = VW980::make_unique<T>();
  ...
  auto test_learner = VW980::LEARNER::make_bottom_learner(
      ...
      "test_learner", VW980::prediction_type_t::SCALAR, VW980::label_type_t::CB)
    ...
    uint32_t num_leaves, uint32_t bandwidth)
{
  ...
  VW980::reductions::cats::cats_tree tree;
...
bool operator!=(const VW980::simple_label& lhs, const VW980::simple_label& rhs) { return !(lhs.label == rhs.label); }

bool operator!=(const VW980::simple_label_reduction_features& lhs, const VW980::simple_label_reduction_features& rhs)
{
...
TEST(CatsTree, OtcAlgoLearn1ActionTillRoot)
{
  ...
  ec.l.cb = VW980::cb_label();
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 2, 0.5f});

  tree.learn(*base, ec);

  // verify 1) # of calls to learn 2) passed in labels 3) passed in weights
  vector<VW980::simple_label> expected_labels = {{-1}, {1}};
...
TEST(CatsTree, OtcAlgoLearn1Action)
{
  ...
  // verify 1) # of calls to learn 2) passed in labels 3) passed in weights
  vector<VW980::simple_label> expected_labels = {{-1}};
...
TEST(CatsTree, OtcAlgoLearn2ActionSiblings)
{
  ...
  ec.debug_current_reduction_depth = 0;
  ec.l.cb = VW980::cb_label();
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 3, 0.5f});
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 4, 0.5f});
...
TEST(CatsTree, OtcAlgoLearn2ActionNotSiblings)
{
  ...
  // verify 1) # of calls to learn 2) passed in labels 3) passed in weights
  vector<VW980::simple_label> expected_labels = {{-1}, {1}, {1}, {1}};
...
TEST(CatsTree, OtcAlgoLearn2ActionNotSiblingsBandwidth1)
{
  ...
  // verify 1) # of calls to learn 2) passed in labels 3) passed in weights
  vector<VW980::simple_label> expected_labels = {
      {1},
      {1},
      {1},
  };
...
TEST(CatsTree, OtcAlgoLearn2ActionSeparate)
{
  ...
  ec.l.cb = VW980::cb_label();
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 3, 0.5f});
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 6, 0.5f});
  ...
  // verify 1) # of calls to learn 2) passed in labels 3) passed in weights
  vector<VW980::simple_label> expected_labels = {{-1}, {1}, {-1}};
  vector<float> expected_weights = {3.5f / 0.5f, 3.5f / 0.5f, 3.5f / 0.5f};
...
TEST(CatsTree, OtcAlgoLearn2ActionSeparate2)
{
  ...
  ec.l.cb = VW980::cb_label();
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 3, 0.5f});
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 7, 0.5f});
...
TEST(CatsTree, OtcAlgoLearn2ActionSeparate2Bandwidth2)
{
  ...
  ec.l.cb = VW980::cb_label();
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 3, 0.5f});
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 11, 0.5f});
...
TEST(CatsTree, OtcAlgoLearn2ActionSeparateBandwidth1Asym)
{
  ...
  ec.l.cb = VW980::cb_label();
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 2, 0.5f});
  ec.l.cb.costs.push_back(VW980::cb_class{3.5f, 5, 0.5f});
...
{
  VW980::reductions::cats::min_depth_binary_tree tree;
  tree.build_tree(4, 1);
  std::vector<VW980::reductions::cats::tree_node> expected = {
...
TEST(CatsTree, BuildMinDepthTreeCont1)
{
  VW980::reductions::cats::min_depth_binary_tree tree;
  tree.build_tree(1, 0);
  std::vector<VW980::reductions::cats::tree_node> expected = {{0, 0, 0, 0, 0, false, false, true}};
  EXPECT_THAT(tree.nodes, ::testing::ContainerEq(expected));
}
