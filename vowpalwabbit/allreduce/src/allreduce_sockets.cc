...
socket_t VW980::all_reduce_sockets::sock_connect(const uint32_t ip, const int port, VW980::io::logger& logger)
{
  ...
  {
    count++;
    logger.err_error("connection attempt {0} failed: {1}", count, VW980::io::strerror_to_string(errno));
#ifdef _WIN32
    Sleep(1);
#else
    sleep(1);
...
socket_t VW980::all_reduce_sockets::getsock(VW980::io::logger& logger)
{
  socket_t sock = socket(PF_INET, SOCK_STREAM, 0);
#ifdef _WIN32
  ...
  {
    logger.err_error("setsockopt SO_REUSEADDR: {}", VW980::io::strerror_to_string(errno));
  }
#endif
  ...
  {
    logger.err_error("setsockopt SO_KEEPALIVE: {}", VW980::io::strerror_to_string(errno));
  }

  return sock;
}

void VW980::all_reduce_sockets::all_reduce_init(VW980::io::logger& logger)
{
#ifdef _WIN32
  WSAData wsaData;
  ...
  if (kid_count > 0)
  {
    ...
    while (!listening)
    {
      ...
      else
      {
        ...
        {
          logger.err_error("Listen: {}", VW980::io::strerror_to_string(errno));
          CLOSESOCK(sock);
          sock = getsock(logger);
        }
        else { listening = true; }
  ...
  else
  {
    ...
    {
      logger.err_error("Read parent_ip={0}(inet_ntop: {1})", parent_ip, VW980::io::strerror_to_string(errno));
    }
...
void VW980::all_reduce_sockets::pass_down(char* buffer, const size_t parent_read_pos, size_t& children_sent_pos)
{
...
void VW980::all_reduce_sockets::broadcast(char* buffer, const size_t n)
{
  ...
  while (parent_read_pos < n || children_sent_pos < n)
  {
    ...
    if (_socks.parent != -1)
    {
      ...
      if (read_size == -1) { THROW("recv from parent: " << VW980::io::strerror_to_string(errno)); }
